...
interface StudentAddContextInterface {
...
const StudentAddContext = createContext<null | StudentAddContextInterface>(null)

type StudentAddContextProps = { children: React.ReactNode }

export const StudentAddContextProvider = ({ children }: StudentAddContextProps) => {
   ...
   * add to the sTUDENT
   */
  // const { control: studentControl, handleSubmit: studentHandleSubmit } = useForm()
  const { control: studentControl, handleSubmit: studentHandleSubmit } = useForm()
  const onSubmitStudentData = async (formData: any) => {
    console.log("pass_photo, join_photo", imageName, toyName, useData?.anganwadi_id)
    const jsonData = {
      anganwadi_id: useData?.anganwadi_id,
      ...formData,
      join_photo: imageName,
      pass_photo: imageName2,
    }
    console.log("sadads", jsonData)
  ...
  //   }
  // }
  const value: StudentAddContextInterface = {
  ...
  return <StudentAddContext.Provider value={value}>{children}</StudentAddContext.Provider>
}

export const useStudentAddContext = () => {
  const context = useContext(StudentAddContext)
  if (!context) throw new Error("use StudentAdd context use in StudentAdd context provider")
  return context
}
