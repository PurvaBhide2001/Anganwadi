...
interface BirthRateProps extends AppStackScreenProps<"BirthRate"> {}

export const BirthRateScreen: FC<BirthRateProps> = observer(function BirthRate({ navigation }) {
  const { onChange, onChangeMonth, showMonthPicker, showMonth } = useCommanContextContext()
  ...
  useHeader({
    ...
    onLeftPress(event) {
      birthRateStatus != 200 && birthRateeReset()
      navigation.goBack()
    },
    leftIcon: "back",
  })
    ...
    <>
      ...
      >
        <WarningModal2
          ...
          maxH={"212"}
          yesFun={executeBirthRateApi}
          // yesFun={BirthRateHandleSubmit(onSubmitBirthRateData)}
          yesName={"हो"}
          cancelName={"नाही"}
        ...
        >
          <Spinner visible={isLoadingBirthrate} textContent={""} textStyle={{ color: "#FFF" }} />
                ...
                <Box flex={1} flexDirection="row">
                  <Box flex={10} alignItems="flex-start">
                    <Text fontSize={responsiveFontSize(2.5)} color="grey">
                      {dateFormatRequiredType(dateBirthRateValue, "YYYY-MM")}
                    </Text>
                  </Box>
                  <Box flex={1}>
                    {birthRateStatus != 200 && (
                    ...
                    {showMonth && (
                      <MonthPicker
                        onChange={(event: any, month) => {
                          onChangeMonth(event, month, callBackBirthRatedateSet)
                        }}
                        value={date}
            ...
            <Box flex={1} flexDirection="row">
              ...
              <Box flex={6} padding={responsiveWidth(2)} borderRadius={responsiveWidth(3)}>
                <InputField2
                  ...
                  isDisabled={birthRateStatus == 200 ? true : false}
                  errors={birthRateError}
                />
              </Box>
            </Box>
          </Box>
        ...
        <Box flex={4} flexDirection="row" justifyContent={"flex-end"}>
          {birthRateStatus == 200 ? (
            ...
            <React.Fragment>
              {isLoadingBirthrate ? ( // Display the loader if isLoading is true
                <View
                  style={{
                    flex: 1,
                    justifyContent: "center",
                ...
                <TouchableOpacity
                  ...
                  onPress={BirthRateHandleSubmit(onSubmitBirthRateData)}
                >
                  <Text
                    textAlign="center"
                    color={"white"}
